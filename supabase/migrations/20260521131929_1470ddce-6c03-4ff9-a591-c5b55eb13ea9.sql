...
AS $$
  ...
  ranked AS (
    SELECT
      ...
      COALESCE(NULLIF(p.full_name, ''), split_part(p.email, '@', 1), 'Recruta') AS full_name,
      agg.correct_count
    FROM agg
  ...
  SELECT
    rank_position,
    user_id,
    CASE WHEN show_name THEN full_name ELSE 'Anônimo' END AS display_name,
    correct_count,
...
CREATE OR REPLACE FUNCTION public.get_my_training_rank()
...
AS $$
  ...
  ranked AS (
    SELECT
      ROW_NUMBER() OVER (ORDER BY correct_count DESC, user_id) AS rank_position,
      user_id,
      correct_count
    FROM agg
  )
  SELECT r.rank_position, r.correct_count, (SELECT COUNT(*) FROM ranked)::bigint AS total_users
  FROM ranked r
  WHERE r.user_id = auth.uid();
$$;

GRANT EXECUTE ON FUNCTION public.get_training_ranking(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_my_training_rank() TO authenticated;
